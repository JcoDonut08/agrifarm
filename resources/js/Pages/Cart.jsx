...
import { products, money, marketHref } from '../Components/Storefront/catalog';

export default function Cart() {
    const { cart, count, changeQuantity, saveForLater } = useShop();
    ...
    return <CollectionPage title="Your cart">
        {items.length ? <div className="basket-layout">
            <section className="basket-products" aria-labelledby="basket-items-heading">
                ...
                {items.map((product) => <article className="basket-product" key={product.id}>
                    ...
                    <div className="basket-product-copy">
                        <h3>{product.name}</h3>
                        <Link className="basket-grower" href={marketHref(product.barangay)}><Icon name="pin" size={14} />{product.barangay}</Link>
                        <p className="basket-unit-price">{money(product.price)} <span>/ {product.unit}</span></p>
                        <p className="basket-stock"><Icon name="check" size={14} />{product.stock} {product.unit === 'bunch' ? 'bunches' : product.unit === 'head' ? 'heads' : 'kg'} available</p>
                    </div>
            ...
            <aside className="basket-summary" aria-labelledby="basket-summary-heading"><h2 id="basket-summary-heading"><Icon name="receipt" size={21} />Order summary</h2><dl><div><dt>Produce subtotal</dt><dd>{money(total)}</dd></div><div><dt>Delivery</dt><dd>Not available yet</dd></div><div className="basket-subtotal"><dt>Subtotal</dt><dd aria-live="polite">{money(total)}</dd></div></dl><p className="basket-summary-note">Sample prices. Delivery options and any additional charges will be shown when checkout is available.</p><button className="store-button" disabled><Icon name="lock" size={17} />Checkout coming soon</button><p className="collection-footnote">This is a preview basket. No payment will be taken.</p></aside>
