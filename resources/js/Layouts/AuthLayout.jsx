    ...
    return (
        <GuestLayout authPage>
            <section className="mx-auto flex w-full max-w-7xl flex-1 flex-col items-center justify-center px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-12">
                <div className={`w-full overflow-hidden rounded-[1.75rem] border border-white/90 bg-white/95 p-5 shadow-[0_28px_80px_-30px_rgba(5,44,25,0.65)] backdrop-blur-sm sm:p-8 ${wide ? 'max-w-[35rem]' : 'max-w-[30rem]'}`}>
                    <div className="text-center">
                        {eyebrow && <p className="text-[0.68rem] font-bold uppercase tracking-[0.2em] text-forest-600">{eyebrow}</p>}
                        <h1 className="auth-title mt-2 text-[1.8rem] font-semibold leading-[1.12] tracking-[-0.025em] text-forest-950 sm:text-[2.15rem]">{title}</h1>
